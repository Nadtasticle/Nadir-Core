...
#include "ui_znadcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->znadTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->znadTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zNAD.";
            else if (!mint.isSeedCorrect)
                strReason = "The zNAD seed used to mint this zNAD is not the same as currently hold in the wallet";
            else
