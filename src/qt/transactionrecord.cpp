...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZNADModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->znadTracker->HasMintTx(hash))) {
            //zNAD stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZNAD;
...
bool IsZNADType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZNAD:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZNAD || type == TransactionRecord::MNReward) {
