...
CzNADWallet::CzNADWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing znad seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNADSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNADSeed_deprecated()) {
                    LogPrintf("%s: Updated zNAD seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old znad seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of znad wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNADWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNADCount(nCountLastUsed);
    else if (!walletdb.ReadZNADCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNADWallet::Lock()
{
    seedMaster = 0;
}

void CzNADWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNADWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNAD(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNADWallet::LoadMintPoolFromDB()
{
...
void CzNADWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNADWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNADWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->znadTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNADWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNAD(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to znadTracker which also adds to database
    pwalletMain->znadTracker->Add(dMint, true);
...
void CzNADWallet::SeedToZNAD(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNADWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNADWallet::GenerateDeterministicZNAD(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNADWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNADWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNADWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
