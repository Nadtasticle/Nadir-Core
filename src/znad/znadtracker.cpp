...
CzNADTracker::CzNADTracker(std::string strWalletFile)
{
...
CzNADTracker::~CzNADTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNADTracker::Init()
{
...
bool CzNADTracker::Archive(CMintMeta& meta)
{
...
bool CzNADTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNADTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNADTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNADTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzNADTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzNADTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzNADTracker::GetSerialHashes()
{
...
CAmount CzNADTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNADTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNADTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNADTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNADTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNADTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNADTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNADTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNADTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNADTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNADTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNADWallet* zNADWallet)
{
    bool iszNADWalletInitialized = (NULL != zNADWallet);
    CMintMeta meta;
    ...
    if (! iszNADWalletInitialized)
        zNADWallet = new CzNADWallet(strWalletFile);
    meta.isSeedCorrect = zNADWallet->CheckSeed(dMint);
    if (! iszNADWalletInitialized)
        delete zNADWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNADTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNADTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNADTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNADTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNADTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNADTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzNADWallet* zNADWallet = new CzNADWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zNADWallet);
        }
        delete zNADWallet;
        LogPrint("zero", "%s: added %d dznad from DB\n", __func__, listDeterministicDB.size());
    }
