...
public:
    ...
    bool WriteZNADSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZNADSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZNADSeed_deprecated(uint256& seed);
    bool EraseZNADSeed();
    bool EraseZNADSeed_deprecated();

    bool WriteZNADCount(const uint32_t& nCount);
    bool ReadZNADCount(uint32_t& nCount);
