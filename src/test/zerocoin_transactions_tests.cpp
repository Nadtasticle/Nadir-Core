...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.znadTracker = unique_ptr<CzNADTracker>(new CzNADTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZNAD_TRX_FUNDS_PROBLEMS, strprintf("Failed Invalid Amount Check: %s", receipt.GetStatusMessage()));
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZNAD_TRX_FUNDS_PROBLEMS, strprintf("Failed Invalid Amount Check: %s", receipt.GetStatusMessage()));
...
BOOST_AUTO_TEST_CASE(zerocoin_public_spend_test)
{
    ...
    CTxIn in;
    if (!ZNADModule::createInput(in, mint, tx.GetHash())){
        BOOST_CHECK_MESSAGE(false, "Failed to create zc input");
    }

    PublicCoinSpend publicSpend(ZCParams);
    if (!ZNADModule::validateInput(in, out, tx, publicSpend)){
        BOOST_CHECK_MESSAGE(false, "Failed to validate zc input");
    }

    PublicCoinSpend publicSpendTest(ZCParams);
    BOOST_CHECK_MESSAGE(ZNADModule::parseCoinSpend(in, tx, out, publicSpendTest), "Failed to parse public spend");
    ...
    BOOST_CHECK_MESSAGE(!ZNADModule::validateInput(in, out, tx, publicSpend2), "Different denomination");
