...
// The zNAD block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zNAD is a hash of the serial
...
{
    //Create an output returning the zNAD that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZNADOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zNAD output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zNAD", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZNADOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zNAD output", __func__);
...
{
    CzNADTracker* znadTracker = pwallet->znadTracker.get();
    CMintMeta meta;
    if (!znadTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    znadTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a NAD stake is the outpoint
