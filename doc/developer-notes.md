...
- [Development guidelines](#development-guidelines)
    - [General NADIR Core](#general-nadir-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
NADIR Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play NADs (tNAD)" on the test network, if you
...
NADIR Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/nadir-project/nadir/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_nadir
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_nadir --log_level=test_suite
$ valgrind -v --leak-check=full src/nadird -printtoconsole
```
...
# A coverage report will now be accessible at `./test_nadir.coverage/index.html`.
```
...
- BitcoinMiner : Generates NADs (if wallet is enabled).
...
However, in open source software such as NADIR Core, where everyone uses
...
pay attention to for reviewers of NADIR Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: NADIR Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against NADIR Core so that they can be integrated
...
file descriptors it will cause problems with NADIR's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:NADIR-Project/NADIR.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `nadir-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `nadir-cli` and the GUI debug console use this table to determine how to
