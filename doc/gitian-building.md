...
*Setup instructions for a gitian build of NADIR using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the NADIR
...
Clone the git repositories for nadir and gitian and then checkout the nadir version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/nadir-crypto/nadir.git
cd nadir
...
Follow the instructions in [doc/release-process.md](release-process.md) in the nadir repository
...
To build NADIR (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the nadir repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/nadir/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/nadir-crypto/nadir
...
For example:
```bash
URL=https://github.com/crowning-/nadir.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit nadir=${COMMIT} --url nadir=${URL} ../nadir/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit nadir=${COMMIT} --url nadir=${URL} ../nadir/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit nadir=${COMMIT} --url nadir=${URL} ../nadir/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/nadir-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/nadir-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/nadir-build.assert
```
...
[nadir/gitian.sigs](https://github.com/nadir-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
